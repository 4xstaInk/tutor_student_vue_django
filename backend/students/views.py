...
from students.serializers import StudentSerializer, SecondaryStudentSerializer
...
def student_list(request):
    if request.method == 'GET':
        ...
        students_serializer = SecondaryStudentSerializer(students, many=True)
        return JsonResponse(students_serializer.data, safe=False)
        # 'safe=False' for objects serialization
 
    elif request.method == 'POST':
        student_data = JSONParser().parse(request)
        student_serializer = SecondaryStudentSerializer(data=student_data, context=student_data)
...
def student_detail(request, pk):
    ...
    if request.method == 'GET': 
        ...
        student_data = JSONParser().parse(request) 
        student_serializer = SecondaryStudentSerializer(student, data=student_data) 
        if student_serializer.is_valid(): 
            student_serializer.save() 
            return JsonResponse(student_serializer.data) 
        return JsonResponse(student_serializer.errors, status=status.HTTP_400_BAD_REQUEST) 
