<template>
    ...
    >
        <span class="flex text-2xl text-red-500 font-lg rounded-lg p-2 cursor-pointer"><mdi:delete class="mr-2" /> Delete all students</span>
    </button>
     <!--Edit Modal -->
    ...
    <div class="max-w-sm mx-auto rounded-lg overflow-hidden shadow-lg bg-white m-4">
      <div class="flex flex-col min-h-full">
        <div class="px-6 py-4 border-b">
          <div class="flex text-xl text-center "><mdi:alert class="mr-2" />Delete all students</div>
        </div>
        <div class="px-6 py-10 flex-grow">
          <p class="text-black text-2xl font-bold">
            Are you sure you want to delete students, total number of <span class="text-green-700">{{data.length}}</span>?
          </p>
...
export default {
    ...
    data() {
     ...
     methods: {
       DeleteAllStudents() {
      axios
        .delete(`http://localhost:8080/api/students`,{
          headers: {
    ...
    reloadPage(){
        this.$emit('update-page','All students deleted');
