<template>
    <div>
        <div class="text-2xl mt-5 mb-2 ml-10">
            ...
            <router-link class="text-blue-500" :to="{name:'teachers'}">Teachers</router-link>\ {{data.fullname}}
            </span>
        </div>
...
<div class="w-8/12 mt-5 mb-5 mx-auto justify-center">
    <div class="flex flex-col rounded-xl bg-white border shadow-md">
        ...
        <section class="font-normal text-2xl text-gray-700 p-3">
          ...
          <p>Gender: <span class="font-bold">{{data.sex}}</span></p>
          <p>Marital status: <span class="font-bold">{{data.marital_status}}</span></p>
...
<div class="w-8/12 mt-5 mb-5 mx-auto justify-center">
    <div class="flex flex-col rounded-xl bg-white border shadow-md">
        ...
        <hr/>
        <section class="font-normal text-2xl text-gray-700 p-3">
          <p>Field: <span class="font-bold">{{data.field}}</span></p>
           <p>Courses: <span class="font-bold">{{data.courses}}</span></p>
        </section>
    </div>
</div>
...
<div title="Click to view more detail data">
  ...
  <transition name="pop" appear>
    <div class="modal" 
         ...
         >
          <div>
         <EditTeacher :data="data" @hide-modal="showModal = false;fetchTeacherData()" />
         </div>
        </div>
  </transition>
...
import DeleteTeacher from "../components/teachers/DeleteTeacher.vue";
export default {
    components: {
        Students,
        EditTeacher,
    ...
    fetchTeacherData() {
        const options = {
          method: "GET",
          url: `http://localhost:8080/api/tutors/${this.routeId}`,
          headers: {
